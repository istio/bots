...
"use strict";const gitHubTokenCookie="gitHubToken";class Authenticator{constructor(){this.gitHubToken=readCookie(gitHubTokenCookie),this.userLogin=null,this.userName=null,this.userAvatarUrl=null,this.onLogin=new LiteEvent,this.onLoginError=new LiteEvent,this.onLogout=new LiteEvent,null!==this.gitHubToken&&this.fetchUserData(this.gitHubToken)}Login(){const a=new netlify.default({});a.authenticate({provider:"github",scope:"user,repo"},(a,b)=>{if(null!==a)return void this.onLoginError.trigger(a);const c=b.token;createCookie(gitHubTokenCookie,c),this.fetchUserData(c)})}fetchUserData(a){fetch("https://api.github.com/user",{headers:{Authorization:"token "+a}}).then(a=>a.text()).then(b=>{const c=JSON.parse(b);this.gitHubToken=a,this.userLogin=c.login,this.userName=c.name,this.userAvatarUrl=c.avatar_url,this.onLogin.trigger(this.userName)})}Logout(){this.gitHubToken=null,this.userName=null,this.userAvatarUrl=null,deleteCookie(gitHubTokenCookie),this.onLogout.trigger()}get LoggedIn(){return this.onLogin.expose()}get LoginError(){return this.onLoginError.expose()}get LoggedOut(){return this.onLogout.expose()}get IsLoggedIn(){return null!==this.gitHubToken}get GitHubToken(){return this.gitHubToken}get UserLogin(){return this.userLogin}get UserName(){return this.userName}get UserAvatarUrl(){return this.userAvatarUrl}}
"use strict";class LiteEvent{constructor(){this.handlers=[]}on(a){this.handlers.push(a)}off(a){this.handlers=this.handlers.filter(b=>b!==a)}trigger(a){this.handlers.slice(0).forEach(b=>b(a))}expose(){return this}}
"use strict";const loginButton=getById("login-button"),logoutButton=getById("logout-button"),authenticator=new Authenticator;authenticator.LoggedIn.on(()=>{resetAuthButtons(),syncUserData()}),authenticator.LoggedOut.on(()=>{resetAuthButtons(),syncUserData()}),authenticator.LoginError.on(a=>{console.log("Login error:",a)});function syncUserData(){const a=document.getElementById("policybot"),b=document.getElementById("name"),c=document.getElementById("image");authenticator.IsLoggedIn?(fetch("https://policybot.istio.io/repos",{}).then(a=>a.text()).then(b=>{a.innerText=b}),b.innerText=authenticator.UserName,c.innerHTML="<img style='width:40px' src='"+authenticator.UserAvatarUrl+"'>"):(a.innerText="Not logged in",b.innerText="",c.innerHTML="")}null!==loginButton&&listen(loginButton,click,a=>{a.preventDefault(),authenticator.Login()}),null!==logoutButton&&listen(logoutButton,click,a=>{a.preventDefault(),authenticator.Logout()});function resetAuthButtons(){authenticator.IsLoggedIn?(null!==loginButton&&(loginButton.style.display="none"),null!==logoutButton&&(logoutButton.style.display="inline-block")):(null!==loginButton&&(loginButton.style.display="inline-block"),null!=logoutButton&&(logoutButton.style.display="none"))}resetAuthButtons(),syncUserData();
