...
"use strict";const gitHubTokenCookie="gitHubToken";class Authenticator{constructor(){this.gitHubToken=readCookie(gitHubTokenCookie),this.userLogin=null,this.userName=null,this.userAvatarUrl=null,this.onLogin=new LiteEvent,this.onLoginError=new LiteEvent,this.onLogout=new LiteEvent}Login(){const a=new netlify.default({});a.authenticate({provider:"github",scope:"user,repo"},(a,b)=>null===a?void(this.gitHubToken=b.token,createCookie(gitHubTokenCookie,this.gitHubToken),fetch("https://api.github.com/user",{headers:{Authorization:"token "+this.gitHubToken}}).then(a=>a.text()).then(a=>{const b=JSON.parse(a);this.userLogin=b.login,this.userName=b.name,this.userAvatarUrl=b.avatar_url,this.onLogin.trigger(this.userName)})):void this.onLoginError.trigger(a))}Logout(){this.gitHubToken=null,this.userName=null,this.userAvatarUrl=null,deleteCookie(gitHubTokenCookie),this.onLogout.trigger()}get LoggedIn(){return this.onLogin.expose()}get LoginError(){return this.onLoginError.expose()}get LoggedOut(){return this.onLogout.expose()}get GitHubToken(){return this.gitHubToken}get UserLogin(){return this.userLogin}get UserName(){return this.userName}get UserAvatarUrl(){return this.userAvatarUrl}}
...
"use strict";const loginButton=getById("login-button"),logoutButton=getById("logout-button"),authenticator=new Authenticator;authenticator.LoggedIn.on(a=>{resetAuthButtons(),fetch("https://policybot.istio.io/repos",{}).then(a=>a.text()).then(a=>{const b=document.getElementById("policybot");b.innerText=a});const b=document.getElementById("name"),c=document.getElementById("image");b.innerText=a,c.innerHTML="<img style='width:40px' src='"+authenticator.UserAvatarUrl+"'>"}),authenticator.LoggedOut.on(()=>{resetAuthButtons();const a=document.getElementById("policybot"),b=document.getElementById("name"),c=document.getElementById("image");a.innerText="",b.innerText="",c.innerHTML=""}),authenticator.LoginError.on(a=>{console.log("Login error:",a)}),null!==loginButton&&listen(loginButton,click,a=>{a.preventDefault(),authenticator.Login()}),null!==logoutButton&&listen(logoutButton,click,a=>{a.preventDefault(),authenticator.Logout()});function resetAuthButtons(){null===authenticator.GitHubToken?(null!==loginButton&&(loginButton.style.display="inline-block"),null!=logoutButton&&(logoutButton.style.display="none")):(null!==loginButton&&(loginButton.style.display="none"),null!==logoutButton&&(logoutButton.style.display="inline-block"))}resetAuthButtons();
