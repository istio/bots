...
class Authenticator {
    ...
    public Login() {
        ...
        authenticator.authenticate({provider: "github", scope: "user,repo"}, (err: string, data: any) => {
            if (err !== null) {
                this.onLoginError.trigger(err);
                return;
            }

            const gitHubToken = data.token as string;
            createCookie(gitHubTokenCookie, gitHubToken);
            ...
            fetch('https://api.github.com/user', {
                ...
                .then(data => {
                    ...
                    console.log("triggering onlogin event");
