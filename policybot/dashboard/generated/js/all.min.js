...
"use strict";const gitHubTokenCookie="gitHubToken";class Authenticator{constructor(){this.gitHubToken=readCookie(gitHubTokenCookie),this.userLogin=null,this.userName=null,this.userAvatarUrl=null,this.onLogin=new LiteEvent,this.onLoginError=new LiteEvent,this.onLogout=new LiteEvent,console.log("Authenticator: 1");const a=new URLSearchParams(window.location.search),b=a.get("access_token");console.log("token: ",b),null!==b&&(this.gitHubToken=b,createCookie(gitHubTokenCookie,this.gitHubToken),console.log("created cookie")),null!==this.gitHubToken&&this.fetchUserData(this.gitHubToken)}Login(){window.location.pathname="login"}Logout(){this.gitHubToken=null,this.userName=null,this.userAvatarUrl=null,deleteCookie(gitHubTokenCookie),this.onLogout.trigger()}get LoggedIn(){return this.onLogin.expose()}get LoginError(){return this.onLoginError.expose()}get LoggedOut(){return this.onLogout.expose()}get IsLoggedIn(){return null!==this.gitHubToken}get GitHubToken(){return this.gitHubToken}get UserLogin(){return this.userLogin}get UserName(){return this.userName}get UserAvatarUrl(){return this.userAvatarUrl}fetchUserData(a){fetch("https://api.github.com/user",{headers:{Authorization:"token "+a}}).then(a=>a.text()).then(b=>{const c=JSON.parse(b);this.gitHubToken=a,this.userLogin=c.login,this.userName=c.name,this.userAvatarUrl=c.avatar_url,this.onLogin.trigger(this.userName)})}}
"use strict";const authnButton=getById("authn-button"),authenticator=new Authenticator;authenticator.LoggedIn.on(resetAuthnButton),authenticator.LoggedOut.on(resetAuthnButton),authenticator.LoginError.on(()=>{}),null!==authnButton&&listen(authnButton,click,a=>{a.preventDefault(),authenticator.IsLoggedIn?authenticator.Logout():authenticator.Login()});function resetAuthnButton(){authnButton&&(authenticator.IsLoggedIn?(authnButton.innerHTML="<img class='large-icon' src='"+authenticator.UserAvatarUrl+"'>",authnButton.title=authenticator.UserName+"\nSign out from GitHub"):(authnButton.innerHTML="<svg class=\"large-icon\"><use xlink:href=\"/icons/icons.svg#github\"/></svg>",authnButton.title="Sign in to GitHub"))}resetAuthnButton();
