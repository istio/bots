...
var _allHtml = []byte(`<aside class="callout warning">
    <div class="type">
...
<script>
    ...
    function getMaintainers() {
        let url = "ws://" + window.location.host + "/maintainersapi/";
        if (window.location.protocol === "https:") {
            url = "wss://" + window.location.host + "/maintainersapi/";
        }
...
func allHtmlBytes() ([]byte, error) {
	return _allHtml, nil
}

func allHtml() (*asset, error) {
	bytes, err := allHtmlBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "all.html", size: 0, mode: os.FileMode(0), modTime: time.Unix(0, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _singleHtml = []byte(`<p>
...
{{ range .RepoInfo }}
    Repo: {{ .Name }}<br/>
    Last Issue Commented: {{ .LastIssueCommented }}<br/>
    Last Issue Closed: {{ .LastIssueClosed }}<br/>
    Last Issue Triaged: {{ .LastIssueTriaged }}<br/>

    {{ range .LastPullRequestActions }}
        Path: {{ .Path }} {{ .When }}<br/>
    {{ end }}
...
var _bintree = &bintree{nil, map[string]*bintree{
	"all.html": &bintree{allHtml, map[string]*bintree{}},
